...
vignette: >
  %\VignetteIndexEntry{HultenRangeMaps-vignette}
...
<br>
To aid spatial analyses of our data, we also provide an example on how to match the digitized distribution maps to the Swedish 10×10 km National grid.  
<br>
...
<br>
A detailed description of how the original atlas was compiled as well as the method used to georeference and digitize the distribution maps can be found in Arnell et al. (xxxx). The dataset can be found at XXXX.  
<br>
...
<br>
This problem was reduced by aligning the images using the plugin program bUnwarpJ (Sorzano et al. 2005, Arganda-Carreras et al. 2006) in the image processing program Fiji ImageJ (Schindelin et al. 2012). The script used to align the maps can be found here (link to GitHub). 
<br>
...
The maps were printed in only two colors (see the map above and Figure 1-7 in Arnell et al. XXX) with distribution areas represented in red and geographical borders represented in green. We can therefore extract the range data from the georeferenced raster images by extracting the red pixel values (Figure 8 et al. XXX).  
<br>  
The function mapDig() takes a scanned range map (raster stack) with three bands (Red-Green-Blue) and extracts the range information by taking the green raster band and setting all pixel values up to 150 to 1 and all other to 0. The pixel range 0-150 in green raster band represents the dark red areas in the scanned map.  
<br>  

```{r,out.width="50%"}
dig_map <- mapDig(ref_map) # digitise the georeferenced map
raster::plot(dig_map, col="black",legend = FALSE) # plot map
```
...
raster::plot(clean_map, col="black")
```

<br>
## Manual data cleaning and updated scientific nomenclature
The cleaned map still contains digitization errors (parts of map border), as well as map symbols that represent non-presence points, e.g. locally extinct populations (open circle) and fossil records (crosses). These have been manually removed from the maps in the database. Please refer to the description in Arnell et al. XXX and the file HULTEN_DataDescription.txt in the data base (Arnell et al. XXX) for information on manual editing as well as updated scientific nomenclature.  
<br>

## Example: how to match the digitized distribution maps to the Swedish National grid 
To aid spatial analyses of our data, we provide an example on how to match the digitized distribution maps to the Swedish 10×10 km National grid, commonly employed in Swedish regional plant inventories. The outcome is a text file with historical presence per grid cell per species.
<br>  
...
The function rangeToSweNationalGrid() performs the steps described above. The function is designed to take a file path to the directory where the database (XXX) is stored. In this example we save an example database map of *Lycopodium alpinum* in a temporary directory.  
<br>  
...
# for the function to work the grid needs to have only two attribute columns: 1 id column and 1 column with names of the grid cells 
...
list_histRange <- rangeToSweNationalGrid(paste0(temp,"\\"))
...
```
```{r}
utils::head(hist_pres)
```
<br>

## Historical biodiversity of vascular plants in Sweden
In the example above we match the historical distribution area of one species to the Swedish national grid. Included in the database (and in this package) is a file with per grid historical presence of all species in the database 'histPresSweNationalGrid.csv' produced using the method above. This can be used to produce a map of the Historical biodiversity of vascular plants in Sweden.  
<br>
For this purpose we exclude grid cells with 50% or more land surface outside Sweden’s national borders and grid cells covered to 80% or more by sea surface or waterways.  
<br>
```{r,out.width="50%"}
...
hist_biodiv <- stats::setNames(stats::aggregate(histPres ~ gridID, histPresSweNationalGrid, sum),c("gridID", "histBiodiv"))
...
```
## Another example: match the digitized distribution maps to the European 10x10 grid in Scandinavia
```{r,out.width="50%",results='hide',message=FALSE}

# new temporary directory
#temp <- base::tempdir()

# add Galuim species ranges in the temprary folder
#for (i in names(galium_list)){
#  write_sf(galium_list[[i]],paste0(temp,"\\",i))
#}
...
#list_histRangeScan <- rangeToSweNationalGrid(paste0(temp,"\\"),pattern="*Galium*", grid = EUgridScandinavia)
...
```{r,out.width="50%",results='hide',message=FALSE}
# count number of species per grid
#hist_biodiv_scan <- stats::setNames(stats::aggregate(histPres ~ gridID, hist_pres_scan, sum),c("gridID", "histBiodiv"))

# add historical biodiversity data to map (this will also take some time...)
#histBiodivScan <- merge(EUgridScandinavia, hist_biodiv_scan)

#plot!
#plot(histBiodivScan["histBiodiv"], border=NA, main="")
```
