...
<br>
To insure reproducability, we have created a 'mini' repository with all the packages that the functions in HultenRangeMaps require. The repository can be downloaded [here](from )https://github.com/ArnellM/HultenRangeMaps_miniCRAN_June2025.git). In the end of this document is a short guide on how to install packages from local repositories.  
<br>
...
list_histRange <- rangeToGrid(paste0(temp,"/"))
...
In this example we store the *Galium* historical range maps in a temporary folder, match their distributions to the European grid and plot the number of *Galium* species per grid in Scandinavia.  
<br>
...
To insure reproducability, we have created a 'miniCRAN' repository with all the packages that the functions in HultenRangeMaps require (and the packages that they in turn require). The repository can be downloaded [here](from )https://github.com/ArnellM/HultenRangeMaps_miniCRAN_June2025.git).  
<br>
